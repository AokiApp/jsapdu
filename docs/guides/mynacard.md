...
### Required Packages
```bash
npm install @aokiapp/pcsc @aokiapp/mynacard @aokiapp/apdu-utils @aokiapp/tlv-parser
```
...
async function readBasicInfo(pin4digit: string) {
  ...
  try {
    ...
    if (verifyResult.sw !== 0x9000) {
      ...
      throw new Error(`PIN verification failed. ${retriesLeft} tries remaining.`);
    }
...
async function verifyCardDataIntegrity(pin4digit: string) {
  ...
  try {
    ...
    const basicHash = await crypto.subtle.digest("SHA-256", basicResponse.arrayBuffer());
    const myNumHash = await crypto.subtle.digest("SHA-256", myNumResponse.arrayBuffer());
...
function handleMynaCardResponse(response: ResponseApdu): void {
  switch (response.sw) {
    ...
    case 0x6982:
      ...
      console.error("Security condition not satisfied - PIN verification required");
      break;
...
async function safeVerifyPin(card: SmartCard, pin: string, ef: number): Promise<boolean> {
  try {
...
describe('MynaCard Operations', () => {
  ...
  it('should handle PIN verification errors', async () => {
    await expect(readBasicInfo("0000")).rejects.toThrow("PIN verification failed");
  });
});
```
