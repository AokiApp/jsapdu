...
export function useHexKeyboardController(): HexKeyboardController {
  ...
  const activeRef = useRef<{ id: InputId | null; onChangeText: (t: string) => void; onBlur?: () => void }>({
    id: null,
    onChangeText: () => {},
  });
  ...
  const open = useCallback((args: OpenArgs) => {
    ...
    activeRef.current = { id: args.id, onChangeText: args.onChangeText, onBlur: args.onBlur };
    setHex(clean);
  ...
  const isActive = useCallback((id: InputId) => activeRef.current.id === id, []);

  const pressNibble = useCallback((n: string) => {
