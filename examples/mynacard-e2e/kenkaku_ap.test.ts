...
const KENKAKU_AP_AID = [0xA0,0x00,0x00,0x00,0x59,0x01,0x02,0x01,0x00,0x01];
const PUBLIC_FILES: { name: string; fid: number[] }[] = [
  { name: 'AP基本情報', fid: [0x00, 0x03] },
...
describe('KENKAKU_AP 公開領域 E2Eテスト', () => {
  it('リーダーが1つ以上取得できる', () => {
        ...
        for (let i = 0; i < deviceInfos.length; i++) {
          try {
            ...
            const selectApdu = new CommandApdu(
              0x00, 0xA4, 0x04, 0x00,
              Uint8Array.from(KENKAKU_AP_AID),
              null
            );
