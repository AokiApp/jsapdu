...
// AIDs from randr.md
const JPKI_AP_AID = [0xD3, 0x92, 0xF0, 0x00, 0x26, 0x01, 0x00, 0x00, 0x00, 0x01]; // JPKI AP
const KENHOJO_AP_AID = [0xD3, 0x92, 0x10, 0x00, 0x31, 0x00, 0x01, 0x01, 0x04, 0x08]; // 券面事項入力補助AP
...
describe('randr.md claims verification', () => {
  ...
  async function acquireCardSession(): Promise<boolean> {
    if (deviceInfos.length === 0) {
      expect.fail('リーダーが見つかりません。スマートカードリーダーが接続されていることを確認してください。');
      return false;
    }
  ...
  describe('特定機関認証関連コマンドの検証 (80 A2)', () => {
    it('JPKI APの特定機関認証EFを選択し、80 A2 00 C1で無意味なデータを流すと、無意味なデータは当然CA証明書による署名検証に失敗し、67 00で失敗する', async () => {
      ...
      // Select JPKI AP
      const selectJpkiApCommand = new CommandApdu(0x00, 0xA4, 0x04, 0x00, Buffer.from(JPKI_AP_AID), 0x00);
      ...
      const command80A200C1 = new CommandApdu(0x80, 0xA2, 0x00, 0xC1, dummyData, 0x00);
  ...
  describe('RSA-PSS署名対応の検証 (80 2A 05 00)', () => {
    it('けんほじょのPINレス署名EFを選択し、80 2A 05 00(JPKI SIGN)で署名を行うと、アルゴリズム05、すなわちPSSで署名できる', async () => {
      ...
      // Select Kenhojo AP
      const selectKenhojoApCommand = new CommandApdu(0x00, 0xA4, 0x04, 0x00, Buffer.from(KENHOJO_AP_AID), 0x00);
      ...
      const command802A0500 = new CommandApdu(0x80, 0x2A, 0x05, 0x00, dummyHash, 0x00); // Le=0x00 means expect all data
  ...
  describe('PIN不要な内部認証の検証 (80 2A 00 00)', () => {
    it('券面事項入力補助APのPINレス署名EFで署名でき、その際にPIN認証がいらないことを検証する', async () => {
      ...
      const command802A0000 = new CommandApdu(0x80, 0x2A, 0x00, 0x00, dummyHash, 0x00); // Le=0x00 means expect all data
