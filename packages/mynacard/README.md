...
async function readBasicInfo(card, pin) {
  ...
  const response = await card.transmit(readEfBinaryFull(KENHOJO_AP_EF.BASIC_FOUR));
...
async function readPersonalInfo(card, pin) {
  ...
  const verifyResult = await card.transmit(verify(pin, { ef: KENHOJO_AP_EF.PIN }));
...
async function verifyDataIntegrity(card, pin) {
  ...
  const sigResponse = await card.transmit(readEfBinaryFull(KENHOJO_AP_EF.SIGNATURE));
  const parser = new SchemaParser(schemaKenhojoSignature);
  const sigData = parser.parse(sigResponse.arrayBuffer());
  
  // Read actual data (実際のデータを読み取り)
  const basicResponse = await card.transmit(readEfBinaryFull(KENHOJO_AP_EF.BASIC_FOUR));
  const myNumResponse = await card.transmit(readEfBinaryFull(KENHOJO_AP_EF.MY_NUMBER));
