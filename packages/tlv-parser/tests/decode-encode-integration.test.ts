...
import { TestData, Decoders, CommonTags, ExpectHelpers, SampleTlvData } from "./test-helpers";
...
describe("TLV Parser -> Builder Integration - Decode then Encode", () => {
  describe("BasicTLVParser -> BasicTLVBuilder roundtrip", () => {
    ...
    test("should decode and encode BOOLEAN values", () => {
      ...
      // Then: Should be identical
      expect(new Uint8Array(reEncodedTrue)).toEqual(new Uint8Array(booleanTrueBuffer));
      expect(new Uint8Array(reEncodedFalse)).toEqual(new Uint8Array(booleanFalseBuffer));
    });
    ...
    test("should decode and encode constructed SEQUENCE", () => {
      ...
      const originalSequence = TestData.createConstructedTlvBuffer(0x30, [child1, child2]);
      ...
      expect(new Uint8Array(reEncoded)).toEqual(new Uint8Array(originalSequence));
    });
  });
  ...
  describe("SchemaParser -> SchemaBuilder roundtrip", () => {
    test("should decode and encode simple primitive with string processing", () => {
      ...
      const originalBuffer = TestData.createTlvBuffer(0x0C, TestData.createStringBuffer("Hello Schema"));
    ...
    test("should decode and encode primitive with number processing", () => {
      ...
      const originalBuffer = TestData.createTlvBuffer(0x02, TestData.createBuffer([0x30, 0x39])); // 12345
    ...
    test("should decode and encode constructed SEQUENCE with multiple fields", () => {
      ...
      const ageData = TestData.createTlvBuffer(0x02, TestData.createBuffer([25]));
      const originalSequence = TestData.createConstructedTlvBuffer(0x30, [nameData, ageData]);

      const parsingSchema = ParserSchema.constructed("person", [
      ...
      const encodingSchema = BuilderSchema.constructed("person", [
        BuilderSchema.primitive<string, string>("name", (data) => TestData.createStringBuffer(data), CommonTags.UTF8_STRING),
        BuilderSchema.primitive<string, number>("age", (data) => TestData.createBuffer([data]), CommonTags.INTEGER)
      ], CommonTags.SEQUENCE);
    ...
    test("should decode and encode nested SEQUENCE structures", () => {
      ...
      const streetData = TestData.createTlvBuffer(0x0C, TestData.createStringBuffer("123 Main"));
      ...
      const originalNested = TestData.createConstructedTlvBuffer(0x30, [nameData, addressSequence]);

      const addressParsingSchema = ParserSchema.constructed("address", [
      ...
      const personParsingSchema = ParserSchema.constructed("person", [
        ParserSchema.primitive("name", (buffer) => new TextDecoder().decode(buffer), CommonTags.UTF8_STRING),
        addressParsingSchema
      ], CommonTags.SEQUENCE);

      const addressEncodingSchema = BuilderSchema.constructed("address", [
        BuilderSchema.primitive<string, string>("street", (data) => TestData.createStringBuffer(data), CommonTags.UTF8_STRING),
        BuilderSchema.primitive<string, string>("city", (data) => TestData.createStringBuffer(data), CommonTags.UTF8_STRING)
      ], CommonTags.SEQUENCE);
    ...
    test("should decode and encode Context-Specific tags in schemas", () => {
      ...
      const field1Data = TestData.createTlvBuffer(0x80, TestData.createStringBuffer("context zero"));
      const field2Data = TestData.createTlvBuffer(0x81, TestData.createStringBuffer("context one"));
      const originalSequence = TestData.createConstructedTlvBuffer(0x30, [field1Data, field2Data]);

      const parsingSchema = ParserSchema.constructed("tagged", [
      ...
      const encodingSchema = BuilderSchema.constructed("tagged", [
        BuilderSchema.primitive<string, string>("field1", (data) => TestData.createStringBuffer(data), CommonTags.CONTEXT_SPECIFIC_0),
        BuilderSchema.primitive<string, string>("field2", (data) => TestData.createStringBuffer(data), CommonTags.CONTEXT_SPECIFIC_1)
      ], CommonTags.SEQUENCE);
    ...
    test("should decode and encode Application tags in schemas", () => {
      ...
      const versionData = TestData.createTlvBuffer(0x41, TestData.createBuffer([2]));
      const dataData = TestData.createTlvBuffer(0x42, TestData.createStringBuffer("app data"));
      const originalApp = TestData.createTlvBuffer(0x60, // Application [0] constructed
        new Uint8Array([...new Uint8Array(versionData), ...new Uint8Array(dataData)]).buffer
      );

      const parsingSchema = ParserSchema.constructed("app", [
      ...
      const encodingSchema = BuilderSchema.constructed("app", [
        BuilderSchema.primitive<string, number>("version", (data) => TestData.createBuffer([data]), CommonTags.APPLICATION_1),
        BuilderSchema.primitive<string, string>("data", (data) => TestData.createStringBuffer(data), CommonTags.APPLICATION_2)
      ], CommonTags.APPLICATION_0);
    ...
    test("should decode and encode Private tags in schemas", () => {
      ...
      const data1Bytes = TestData.createTlvBuffer(0xC0, TestData.createBuffer([0x01, 0x02, 0x03]));
      const data2Bytes = TestData.createTlvBuffer(0xC1, TestData.createStringBuffer("private"));
      const originalPrivate = TestData.createConstructedTlvBuffer(0xCA, // Private [10] with constructed bit
        [data1Bytes, data2Bytes]
      );

      const parsingSchema = ParserSchema.constructed("private", [
        ParserSchema.primitive("data1", (buffer) => new Uint8Array(buffer), CommonTags.PRIVATE_0),
        ParserSchema.primitive("data2", (buffer) => new TextDecoder().decode(buffer), { tagClass: TagClass.Private, tagNumber: 1 })
      ], { tagClass: TagClass.Private, tagNumber: 10 });

      const encodingSchema = BuilderSchema.constructed("private", [
        BuilderSchema.primitive<Uint8Array, Uint8Array>("data1", (data) => data.buffer, CommonTags.PRIVATE_0),
        BuilderSchema.primitive<string, string>("data2", (data) => TestData.createStringBuffer(data), { tagClass: TagClass.Private, tagNumber: 1 })
      ], { tagClass: TagClass.Private, tagNumber: 10 });
      ...
      // Then: Should produce equivalent data
      expect(new Uint8Array(reEncoded)).toEqual(new Uint8Array(originalPrivate));
    });
  });
  ...
  describe("Async schema roundtrip", () => {
    test("should handle async decoding and encoding", async () => {
      ...
      const originalBuffer = TestData.createTlvBuffer(0x0C, TestData.createStringBuffer("async test"));
  ...
  describe("Error handling in roundtrip", () => {
    ...
    test("should validate schema constraints during roundtrip", () => {
      ...
      const mismatchedBuffer = TestData.createTlvBuffer(0x04, TestData.createStringBuffer("test")); // OCTET_STRING
      const schema = ParserSchema.primitive("field", Decoders.string, CommonTags.UTF8_STRING); // Expects UTF8_STRING

      const parser = new SchemaParser(schema);

      // When/Then: Should throw tag mismatch error
      expect(() => parser.parse(mismatchedBuffer)).toThrow(/tag number mismatch/i);
    });
  });
});