...
describe("SchemaBuilder - Synchronous and asynchronous building", () => {
  ...
  test("should build asynchronously when requested", async () => {
    const schema = Schema.primitive<string, string>("asyncField", Encoders.asyncString, CommonTags.UTF8_STRING);
    const builder = new SchemaBuilder(schema);
  ...
  test("should handle mixed sync/async encoding in nested structures", async () => {
    const syncSchema = Schema.primitive<string, string>("sync", Encoders.string, CommonTags.UTF8_STRING);
    const asyncSchema = Schema.primitive<string, string>("async", Encoders.asyncString, CommonTags.UTF8_STRING);
