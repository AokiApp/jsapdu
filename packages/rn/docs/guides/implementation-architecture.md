# 実装アーキテクチャ方針

**📋 注意**: このドキュメントは既存仕様の**補完**です。詳細な設計方針は以下を参照：

- **設計詳細**: [android-nfc-ddd.md](../ddd/android-nfc-ddd.md) - 責務分担・全体構成
- **命名規約**: [nitro-method-conventions.md](../nitro-method-conventions.md) - FFI中立・命名方針
- **技術制約**: [android-nfc-tsd.md](../tsd/android-nfc-tsd.md) - スレッド・同期・制約

---

## 📋 このドキュメントの役割

**既存の包括的設計仕様**を、実装者が迷わず適用できるよう**実践的な観点**で整理します：

- **既存設計** ([android-nfc-ddd.md](../ddd/android-nfc-ddd.md)) の実装時のポイント
- **既存命名規約** ([nitro-method-conventions.md](../nitro-method-conventions.md)) の具体的適用方法
- **ファイル構成の実践的指針**

実装者が迷わないよう、既存仕様との整合性を保ちながら**具体的な実装方針**を示します。

---

## 🏗️ 基本設計方針

### 方針1: Nitro Modules 標準パターンの採用

**決定事項**: React Native Nitro Modules の**標準的なパターン**に従います

**理由**:

- フレームワークの想定する使用方法に沿う
- 将来的なNitro Modulesアップデートへの追従が容易
- デバッグツールやドキュメントとの整合性
- 他のNitro Modulesライブラリとの一貫性

**具体的な影響**:

```
✅ 採用パターン: 単一エントリーポイント
❌ 非採用: 複数モジュールによる分散実装
```

### 方針2: FFI中立性の維持

**決定事項**: 公開APIは**OS固有の概念を一切露出しない**

**理由**:

- iOS版実装時の設計整合性
- ユーザーコードでのプラットフォーム分岐回避
- テストコードの共通化
- ドキュメントの単純化

**具体的な影響**:

```typescript
✅ 適切: SmartCardDevice.waitForCardPresence()
❌ 不適切: AndroidNfcDevice.enableReaderMode()

✅ 適切: DeviceInfo.apduApi = ["nfc", "androidnfc"]
❌ 不適切: AndroidDeviceInfo.readerModeFlags
```

### 方針3: エラーハンドリングの統一

**決定事項**: すべての例外は**SmartCardErrorコード体系**に正規化

**理由**:

- ユーザー側でのエラー処理コード統一
- ログ・監視システムでの分類容易性
- 多言語対応の基盤
- デバッグ情報の体系化

**具体的な影響**:

```kotlin
✅ 適切: throw SmartCardException("PLATFORM_ERROR", "NFC not supported")
❌ 不適切: throw IOException("NFC adapter not found")
```

---

## 📁 ファイル構成方針

### TypeScript側: 責務分離ファイル構成

**決定事項**: **責務に応じた適切なファイル分離**

```
src/
├── JsapduRn.nitro.ts           # Nitroインターフェース定義
├── types/
│   ├── device-info.ts          # デバイス関連型定義
│   └── apdu.ts                  # APDU関連型定義
├── platform/
│   └── smart-card-platform.ts  # プラットフォーム実装
├── device/
│   └── smart-card-device.ts    # デバイス実装
├── card/
│   └── smart-card.ts           # カード実装
├── errors/
│   └── error-mapper.ts         # エラー正規化ロジック
└── index.tsx                   # 公開APIエクスポート
```

**理由**:

- **単一責任原則** (SRP) の遵守
- **テスト容易性**: 各クラスの独立テスト可能
- **保守性**: 変更影響範囲の局所化
- **拡張性**: 新機能追加時の影響最小化
- **チーム開発**: 並行開発・レビュー効率化

### Kotlin側: 責務別クラス分離方式

**決定事項**: **責務別の完全分離クラス構成**

```
android/src/main/java/.../
├── JsapduRn.kt                    # Nitroエントリーポイント (委譲のみ)
├── platform/
│   └── SmartCardPlatformImpl.kt   # プラットフォーム実装
├── device/
│   └── SmartCardDeviceImpl.kt     # デバイス・ReaderMode管理
├── card/
│   └── SmartCardImpl.kt           # カード・IsoDep管理
└── errors/
    └── AndroidErrorMapper.kt      # Android例外→SmartCardError変換
```

**理由**:

- **関心事の分離**: 各クラスが単一の責務に集中
- **テスト戦略**: 各レイヤーの独立テスト
- **保守性向上**: 変更時の影響範囲明確化
- **並行開発**: 複数開発者による同時作業支援

**実装ルール**:

```kotlin
✅ 適切: JsapduRn は純粋な委譲層
override fun initPlatform() = Promise.async {
    platformImpl.initialize(reactContext)
}

❌ 不適切: JsapduRn でビジネスロジック実装
override fun initPlatform() = Promise.async {
    val adapter = NfcAdapter.getDefaultAdapter(reactContext) // 責務混在
}
```

---

## 🔌 依存関係管理方針

### 方針1: 最小限依存の原則

**決定事項**: 外部ライブラリ依存を**最小限**に留める

**許可する依存**:

```kotlin
✅ Android Standard Library (android.nfc.*)
✅ React Native Core (com.facebook.react.*)
✅ Nitro Modules (com.margelo.nitro.*)
```

**禁止する依存**:

```kotlin
❌ サードパーティNFCライブラリ
❌ 追加のJSONライブラリ
❌ ログライブラリ (React Nativeの標準を使用)
❌ 非必須のユーティリティライブラリ
```

**理由**: アプリサイズ増大の回避、依存関係競合の回避、セキュリティリスク低減

### 方針2: バージョン固定の原則

**決定事項**: 重要な依存関係は**バージョンを固定**

```json
// package.json
{
  "dependencies": {
    "react-native-nitro-modules": "0.31.1" // 固定
  },
  "peerDependencies": {
    "react-native": ">=0.74.0" // 最小バージョン指定
  }
}
```

**理由**: 予期しないBreaking Changes回避、ビルド再現性確保

---

## 🧵 スレッド・並行性方針

### 方針1: UI Thread完全回避

**決定事項**: NFC I/Oは**UI Thread以外で実行**を厳守

**実装パターン**:

```kotlin
✅ 適切: Promise.async { } 内でI/O実行
override fun transmit(apdu: ArrayBuffer): Promise<Map<String, Any?>> = Promise.async {
    // ここはBackground Threadで実行される
    isoDep.transceive(command)
}

❌ 不適切: 同期メソッドでI/O実行
override fun transmitSync(apdu: ArrayBuffer): Map<String, Any?> {
    return isoDep.transceive(command) // UI Threadブロックの危険
}
```

### 方針2: 排他制御の局所化

**決定事項**: 排他制御は**ヘルパークラス内部**で完結

```kotlin
class SmartCardDeviceManager {
    private val mutex = Mutex() // 排他制御はここで完結

    suspend fun acquire(...) = mutex.withLock {
        // 排他制御が必要な処理
    }
}

// JsapduRn.kt では排他制御コードを書かない
override fun acquireDevice(deviceId: String) = Promise.async {
    deviceManager.acquire(deviceId, ...) // 委譲するのみ
}
```

**理由**: 複雑性の局所化、テストの容易性、バグ混入リスク低減

---

## 🔄 ライフサイクル管理方針

### 方針1: RAII (Resource Acquisition Is Initialization) パターン

**決定事項**: リソース取得と解放を**対称的**に設計

```typescript
// 対称的なライフサイクル
await SmartCardPlatform.init(); // 取得
await SmartCardPlatform.release(); // 解放

const device = await platform.acquireDevice(id); // 取得
await device.release(); // 解放

const card = await device.startSession(); // 取得
await card.release(); // 解放
```

**強制ルール**:

- すべての `acquire` 系メソッドには対応する `release` を用意
- `release` は冪等性を保証（複数回呼んでもエラーにならない）
- リソースリーク防止のため、finalizer的な仕組みも併用

### 方針2: 例外安全性の保証

**決定事項**: 例外発生時でも**リソースリークしない**設計

```kotlin
class SmartCardManager {
    fun startSession(tag: Tag) {
        val isoDep = IsoDep.get(tag)
        try {
            isoDep.connect()
            this.isoDep = isoDep
            isActive = true
        } catch (e: Exception) {
            isoDep.close() // 例外時のクリーンアップ
            throw SmartCardException("PLATFORM_ERROR", e.message)
        }
    }
}
```

---

## 📏 性能・制約方針

### 方針1: レスポンス性能の基準

**決定事項**: 以下の性能基準を**必須要件**とする

```
✅ 必須基準:
- init(): < 1000ms
- getDeviceInfo(): < 100ms
- acquireDevice(): < 500ms
- transmit(): < 3000ms (APDU dependent)
- release系: < 200ms

⚠️ 注意事項:
- waitForCardPresence(): ユーザー操作依存のためタイムアウトで制御
- 測定は中位性能端末 (Snapdragon 660相当) で実施
```

### 方針2: メモリ使用量制限

**決定事項**: 以下のメモリ制限を**設計原則**とする

```
✅ 制限値:
- ライブラリ実装: < 5MB (常駐)
- APDU バッファ: < 100KB (一時)
- ログバッファ: < 1MB (開発時のみ)

✅ 対策:
- 大きなバッファの即座解放
- WeakReference活用
- Static変数の最小化
```

---

## 🧪 テスト方針

### 方針1: テストピラミッドの適用

**決定事項**: **単体テスト重点、統合テスト補完**の構造

```
🔼 手動テスト (少): 実機でのエンドツーエンド検証
🔳 統合テスト (中): モック環境での全体フロー
🔺 単体テスト (多): 各クラスの個別機能
```

**具体的比率**:

- 単体テスト: 70% (各メソッドの正常系・異常系)
- 統合テスト: 25% (クラス間連携・エラー伝播)
- 手動テスト: 5% (実際のNFCカードでの動作確認)

### 方針2: モック戦略

**決定事項**: **Android NFC API のみモック**、Nitro部分はそのまま

```kotlin
✅ モック対象:
- NfcAdapter
- IsoDep
- Tag

❌ モック対象外:
- Promise (Nitro Modules)
- SmartCardError (自作クラス)
- ArrayBuffer (標準クラス)
```

**理由**: Nitro固有の問題とNFC固有の問題を分離、テスト信頼性向上

---

## 📋 コーディング規約

### 命名規約

```kotlin
✅ クラス名: PascalCase
class SmartCardPlatformManager

✅ メソッド名: camelCase
fun initializePlatform()

✅ 定数: UPPER_SNAKE_CASE
companion object {
    private const val DEFAULT_TIMEOUT_MS = 15000L
}

✅ プライベート変数: camelCase with prefix
private var isInitialized = false
```

### エラーメッセージ規約

```kotlin
✅ ユーザー向けメッセージ: 日本語、解決策提示
throw SmartCardException("PLATFORM_ERROR",
    "NFCがサポートされていません。NFC対応端末でお試しください。")

✅ 開発者向けログ: 英語、技術詳細
Log.d(TAG, "NfcAdapter.getDefaultAdapter() returned null")
```

---

## 🔄 変更管理方針

### 方針1: Breaking Change 最小化

**決定事項**: 公開APIの**Breaking Changes は極力回避**

```typescript
✅ 安全な変更:
- 新しいオプション引数の追加
- 新しいメソッドの追加
- 内部実装の最適化

❌ 危険な変更:
- 既存メソッドの引数変更
- 戻り値型の変更
- エラーコード体系の変更
```

### 方針2: 段階的移行サポート

**決定事項**: 大きな変更は**Deprecation → 削除**の段階を踏む

```typescript
// 例: メソッド名変更の場合
class SmartCardPlatform {
  // 新しいメソッド
  static async initialize(): Promise<void> { ... }

  // 古いメソッド (Deprecated)
  /** @deprecated Use initialize() instead */
  static async init(): Promise<void> {
    console.warn('SmartCardPlatform.init() is deprecated. Use initialize() instead.');
    return this.initialize();
  }
}
```

---

## ✅ 実装完了基準

以下すべてが満たされた場合に**実装完了**とします：

### 必須基準

- [ ] すべてのpublic APIが実装されている
- [ ] 単体テストカバレッジ > 80%
- [ ] 性能基準をすべてクリア
- [ ] メモリリークテストをパス
- [ ] 実機での基本動作確認完了

### 品質基準

- [ ] ドキュメント整合性確認
- [ ] 静的解析エラーゼロ
- [ ] ESLint/Ktlint エラーゼロ
- [ ] 型安全性検証完了

### 出荷基準

- [ ] セキュリティレビュー完了
- [ ] パフォーマンステスト完了
- [ ] 実機互換性テスト完了
- [ ] ユーザビリティテスト完了

---

**この方針に従うことで、保守可能で拡張しやすい高品質な実装を実現します。**
