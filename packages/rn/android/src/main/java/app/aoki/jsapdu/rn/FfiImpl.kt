...
object FfiImpl {
  ...
  fun initPlatform(force: Boolean = false) {
  ...
  fun releasePlatform(force: Boolean = false) {
  ...
  fun reset(deviceHandle: String, cardHandle: String) {
    val platform = ensureInitialized()
    val device: IDevice = platform.getTarget(deviceHandle)
      ?: throw IllegalArgumentException("INVALID_DEVICE_HANDLE: No such device '$deviceHandle'")
    val card: ICardSession = device.getTarget(cardHandle)
  ...
  fun releaseCard(deviceHandle: String, cardHandle: String) {
    val platform = ensureInitialized()
    val device: IDevice = platform.getTarget(deviceHandle)
      ?: throw IllegalArgumentException("INVALID_DEVICE_HANDLE: No such device '$deviceHandle'")
    val card: ICardSession = device.getTarget(cardHandle)
  ...
  fun onStatusUpdate(callback: ((eventType: String, payload: EventPayload) -> Unit)?) {
    if (callback == null) {
