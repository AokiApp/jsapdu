  ...
  fun transmit(apdu: ByteArray): ByteArray {
    ...
    when {
      ...
      isManageChannelClose(cmd) -> {
        ...
        val ch = currentChannel ?: throw IllegalStateException(
          "Channel not available - no logical channel opened. Send SELECT by AID first."
        )
        ...
        if (!ch.isOpen) {
          ...
          throw IllegalStateException(
            "Channel not available - logical channel was closed. Send SELECT by AID to reopen."
          )
        }
